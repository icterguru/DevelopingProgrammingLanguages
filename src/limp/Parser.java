...
public class Parser extends Types {
    ...
    */
    public Lexeme primary() {
        ...
        else if (check(MINUS)){  // unary minus and plus
            tree = match(MINUS);
            tree.type = UMINUS;
            tree.left = null;
            tree.right = primary();
        }
        else { //if (check (PLUS)){  // unary minus and plus
            tree = match(PLUS);
            tree.type = UPLUS;
            tree.left = null;
            tree.right = primary();
    ...
    */
    ...
    public Lexeme expression5() {
        Lexeme tree;
        tree = expression6();
        while (operatorAdditivePending()){ // binary + or -
            Lexeme tmp = operator();
            tmp.left = tree;
    ...
    public Lexeme expression6() {
        Lexeme tree;
        tree = expression7();
        while (operatorMultiplicativePending()){  // * and /
            Lexeme tmp = operator();
            tmp.left = tree;
    ...
    public Lexeme expression7() {
        Lexeme tree;
        tree = expression8();
        if (operatorUnaryPending()){   // unary + or -
            Lexeme tmp = operator();
            tmp.left = tree;
    ...
    public Lexeme expression8() {  
        Lexeme tree;
        tree = primary();
        while (operatorPrimaryPending()){ // checks for .
            Lexeme tmp = operator();
            tmp.left = tree;
            tmp.right = primary();
            tree = tmp;
    ...
    */ 
    ...
    public boolean operatorUnaryPending() {
        return check(UMINUS) || check(UPLUS);
    }
